...
def update_globals(proc_times, targ_times):
    G.process_times = {
        1: proc_times[0],
        2: proc_times[1],
        3: proc_times[2]    
...
def gen_system_state(sim_time, num_rads, arr_times, proc_times, targ_times, constant_rads, cutoff, verbose):
    #Define urgency times
    update_globals(proc_times, targ_times)
    #Create the intervals
    arrivals_dict, arrival_times_tuples_list = create_arrival_times(sim_time, arr_times)
    ...
    radiologists = create_radiologists(num_rads, constant_rads)
...
def sim(sim_time, num_rads, arr_times, proc_times, targ_times, constant_rads=False, cutoff=False, verbose=False):  
    s = gen_system_state(sim_time, num_rads, arr_times, proc_times, targ_times, constant_rads, cutoff, verbose)
    s.run_simulation()    
    return s
...
def wait_time_plot(img_table):
    fig, ax = plt.subplots()
    x = img_table['time_created'].values
    y = img_table['wait_time'].values
    colormap = {
        1: "red",
        2: "orange",
        3: "green"
...
def wait_time_hist(curr_sim):
    ...
    #urgencies seperated
    fig, ax = plt.subplots()
    plt.hist(curr_sim.img_table[curr_sim.img_table.urgency==3].wait_time, label=f"urgency 3: {len(curr_sim.img_table[curr_sim.img_table.urgency==3])} images", color="red", alpha=0.6)
    plt.hist(curr_sim.img_table[curr_sim.img_table.urgency==2].wait_time, label=f"urgency 2: {len(curr_sim.img_table[curr_sim.img_table.urgency==2])} images", color="yellow", alpha=0.5)
    plt.hist(curr_sim.img_table[curr_sim.img_table.urgency==1].wait_time, label=f"urgency 1: {len(curr_sim.img_table[curr_sim.img_table.urgency==1])} images", color="green", alpha=0.5)
    plt.xlabel("time (mins)")
    plt.ylabel("Number of Images")
    plt.title("Time to be processed from creation (All Medical Images)")
    plt.legend()
    ...
    t1 = np.mean(curr_sim.img_table[curr_sim.img_table.urgency==1].wait_time)
    t2 = np.mean(curr_sim.img_table[curr_sim.img_table.urgency==2].wait_time)
    t3 = np.mean(curr_sim.img_table[curr_sim.img_table.urgency==3].wait_time)
